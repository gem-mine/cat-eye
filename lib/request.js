...
function proxy(cfg) {
  ...
  function request(url, options) {
    ...
    var params = _extends({}, defaults, domain_default_config[name], options);
    this.params = params;
    this.request = ins;
...
function init(configs) {
  ...
  Object.keys(configs).forEach(function (key) {
    ...
    proxy[key].config = function (options) {
      domain_default_config[key] = _extends(domain_default_config[key], options);
    };
  });
  return proxy;
}
